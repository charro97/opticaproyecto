...
class Router {
    ...
    public static function dispatch():void {
        ...
        $action = preg_replace("/\/opticaproyecto\/public\//",'',$_SERVER['REQUEST_URI']);
       //$_SERVER['REQUEST_URI'] almacena la cadena de texto que hay después del nombre del host en la URL
        ...
        if(!empty($match)){
            ...
            $param = $match[0];
            $action=preg_replace('/'.$match[0].'/',':id',$action);//quitamos la primera parte que se repite siempre (clinica_new)
        }
        /*
